...
export default function SuperAdminPage() {
  ...
  // Form States (Orgs) - Includes firstName and lastName
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // --- 2. CREATE MODE (Split Names -> Combined for DB) ---
    try {
      const response = await fetch('/api/organizations/create', {
        ...
        body: JSON.stringify({
          name: formData.name,
          contactName: finalContactName, // Sends "First Last"
  ...
  const handleAddSuperAdmin = async (e: React.FormEvent) => {
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ role: 'super admin', full_name: fullName })
      .eq('id', users.id);

    if (updateError) {
      setAdminError("Failed to promote user.");
    } else {
  ...
  const handleDeleteSuperAdmin = async (id: string) => {
    if (confirm("Revoke Super Admin access?")) {
                    ...
                    <tr key={org.id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-6">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-sm font-bold text-slate-300 shrink-0">{org.name.charAt(0)}</div>
                          <div>
                      ...
                      <td className="p-6">
                        ...
                        <div className="text-sm text-slate-500 mt-1">{org.paymentStatus === 'past_due' ? <span className="text-red-400 font-bold">Past Due</span> : 'Auto-Pay'}</div>
                      </td>
                      <td className="p-6 min-w-[150px]">
                        <div className="flex justify-between text-sm mb-2 font-medium"><span className="text-slate-300">{org.storageUsedGB} GB</span><span className="text-slate-500">/ {org.storageLimitGB}</span></div>
                        <div className="w-full h-2.5 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full rounded-full ${org.storageUsedGB / org.storageLimitGB > 0.9 ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${(org.storageUsedGB / org.storageLimitGB) * 100}%` }} /></div>
                      </td>
                      <td className="p-6">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase border tracking-wide ${org.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : org.status === 'suspended' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>{org.status}</span>
                      </td>
          ...
          <div className="xl:col-span-4 space-y-6">
            <div className="glass-panel p-8 bg-gradient-to-b from-slate-900 to-indigo-950/20 border border-white/10">
              <div className="flex justify-between items-start mb-8">
                <div><h3 className="text-xl font-bold text-white">Revenue Analytics</h3><p className="text-slate-400 text-sm mt-1 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> Live from Stripe</p></div>
              </div>
              <div className="mb-8"><span className="text-sm font-bold text-slate-500 uppercase tracking-wider">Total Revenue (YTD)</span><div className="text-5xl font-bold text-white mt-2">$143,850</div><div className="text-emerald-400 text-sm font-bold mt-1">+12.5% vs last year</div></div>
              <div className="h-48 flex items-end justify-between gap-2 mb-4">
                {REVENUE_DATA.map((data, index) => (
                  <div key={index} className="flex-1 flex flex-col items-center group"><div className={`w-full rounded-t-sm ${index === REVENUE_DATA.length - 1 ? 'bg-indigo-500' : 'bg-slate-700'}`} style={{ height: data.height, minHeight: '4px' }}></div></div>
                ))}
              </div>
            </div>
                ...
                {RECENT_TRANSACTIONS.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${tx.status === 'succeeded' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>
                      <div><div className="text-sm font-bold text-white">{tx.org}</div><div className="text-xs text-slate-500">{tx.date}</div></div>
                    </div>
                    <div className="text-sm font-mono font-bold text-white">${tx.amount}</div>
                  </div>
                ))}
              </div>
              <button className="w-full mt-4 py-2 text-sm text-indigo-400 hover:text-indigo-300 font-bold border border-dashed border-white/10 hover:border-indigo-500/30 rounded-lg transition-colors">View All Transactions &rarr;</button>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                    ...
                    {editingOrgId ? (
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input type="text" required className="glass-input w-full bg-slate-950 text-sm p-2" value={formData.firstName || ''} onChange={e => setFormData({...formData, firstName: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Last Name</label>
                                <input type="text" required className="glass-input w-full bg-slate-950 text-sm p-2" value={formData.lastName || ''} onChange={e => setFormData({...formData, lastName: e.target.value})} />
                            </div>
                        </div>
                    )}
                  ...
                  <div className="space-y-3">
                      <div className="p-4 bg-white/5 rounded-xl border border-white/5 space-y-3 h-full">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div><label className="block text-[10px] font-bold text-indigo-300 uppercase mb-1">Storage (GB)</label><input type="number" className="glass-input w-full bg-slate-950 border-indigo-500/30 text-sm p-2" value={formData.storageLimitGB || 0} onChange={e => setFormData({...formData, storageLimitGB: Number(e.target.value)})} /></div>
                            <div><label className="block text-[10px] font-bold text-emerald-300 uppercase mb-1">Fee ($)</label><input type="number" className="glass-input w-full bg-slate-950 border-emerald-500/30 text-sm p-2" value={formData.monthlyFee || 0} onChange={e => setFormData({...formData, monthlyFee: Number(e.target.value)})} /></div>
                        </div>
                        <div className="pt-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="glass-panel w-full max-w-2xl p-6 relative z-50 animate-fade-in border border-white/10 shadow-2xl bg-slate-900 max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-4">
              ...
              <button onClick={() => setIsManageTeamOpen(false)} className="text-slate-400 hover:text-white"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
                ...
                <div className="space-y-2">
                  {superAdmins.map((admin) => (
                    <div key={admin.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold text-sm">{admin.name.charAt(0)}</div>
                        <div><div className="font-bold text-white text-sm flex items-center gap-2">{admin.name}{admin.role === 'owner' && <span className="text-[9px] bg-indigo-500 text-white px-2 py-0.5 rounded-full uppercase">OWNER</span>}</div><div className="text-xs text-slate-400">{admin.email}</div></div>
                      </div>
                      {admin.role !== 'owner' && (<button onClick={() => handleDeleteSuperAdmin(admin.id)} title="Revoke Admin Access" className="p-2 text-slate-500 hover:text-red-400 transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>)}
                    </div>
                  ))}
                </div>
              )}
            ...
            <form onSubmit={handleAddSuperAdmin} className="bg-slate-950/50 p-4 rounded-xl border border-white/10">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div className="col-span-2"><input type="email" placeholder="User Email Address" className="glass-input w-full bg-slate-950 border-white/10 text-sm p-2" value={newAdminData.email} onChange={(e) => setNewAdminData({ ...newAdminData, email: e.target.value })} /></div>
              </div>
              <button type="submit" className="w-full glass-button bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 rounded-lg transition-all disabled:opacity-50 text-sm" disabled={!newAdminData.email}>Grant Access</button>
            </form>
          </div>
        </div>
      )}
